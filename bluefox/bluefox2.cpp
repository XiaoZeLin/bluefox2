...
std::string Bluefox2::AvailableDevice()
{
	DeviceManager dev_mgr_temp;
	const auto dev_cnt = dev_mgr_temp.deviceCount();
	std::string devices = std::to_string(dev_cnt) + " availabe device(s): ";
	for (decltype(dev_mgr_temp.deviceCount()) i = 0; i < dev_cnt; ++i) {
		devices += dev_mgr_temp.getDevice(i)->serial.read() + " ";
	}
	return devices;
}
...
int Bluefox2::RequestSingle() const
{
	int result = DMR_NO_ERROR;
	result = fi_->imageRequestSingle();
	if (result != DMR_NO_ERROR) {
		std::cout << serial() << ": Error while requesting image: " << result
...
int Bluefox2::manuallyStartAcquisitionIfNeeded() const
{
	int result = DMR_NO_ERROR;
	if (dev_->acquisitionStartStopBehaviour.read() == assbUser) {
		if ((result = static_cast<TDMR_ERROR>(fi_->acquisitionStart())) != DMR_NO_ERROR) {
			std::cout << serial() << ": Error while setting acquisitionStart : " << result
					   << "(" << ImpactAcquireException::getErrorCodeAsString(result) << ")"
					   << std::endl;
		}
	}
	else {
		std::cout << "acquisitionStartStopBehaviour is not assbUser" << std::endl;
	}
	return result;
}
/*
...
*/
bool Bluefox2::GrabImage(cv::Mat &image)
{
	...
	case idpf_auto:
  	...
  	case idpf_mono8:
  		...
  		frame = cv::Mat(request_->imageHeight.read(),request_->imageWidth.read(),CV_8UC1,request_->imageData.read());
  		frame.copyTo(image);
  		break;
  	case idpf_mono16:
  		//std::cout<<"mono16"<<std::endl;
  		frame = cv::Mat(request_->imageHeight.read(),request_->imageWidth.read(),CV_16UC1,request_->imageData.read());
  		frame.copyTo(image);
  		break;
  	case idpf_rgb888_packed:
  		//std::cout<<"rgb888_packed"<<std::endl;
  		frame = cv::Mat(request_->imageHeight.read(),request_->imageWidth.read(),CV_8UC3,request_->imageData.read());
  		frame.copyTo(image);
  		break;	
  	case idpf_bgr888_packed:
  		//std::cout<<"bgr888_packed"<<std::endl;
  		frame = cv::Mat(request_->imageHeight.read(),request_->imageWidth.read(),CV_8UC3,request_->imageData.read());
  		frame.copyTo(image);
  		break;
  	default:
